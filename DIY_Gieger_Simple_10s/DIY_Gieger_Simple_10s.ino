...
#define DISP_LAST        10000.0          // (1 sec) ms last count) - DEBUG
...
void setup(){
  ...
  lcd.print("10 sec count");             // display a simple banner
...
void loop(){
  ...
  if (millis() >= dispPeriodStart + DISP_PERIOD){ // DISPLAY PERIOD
    ...
    dispTime = dispTime +(DISP_PERIOD/1000);
